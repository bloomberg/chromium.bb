...
constexpr int32_t LINE_BREAK_ID = 10;
...
class AXRangeTest : public testing::Test, public AXTreeManager {
 public:
  ...
  const base::string16 ALL_TEXT = BUTTON.substr().append(TEXT_FIELD);
...
void AXRangeTest::SetUp() {
  ...
  line_break_.SetName(NEWLINE);
  line_break_.relative_bounds.bounds = gfx::RectF(50, 50, 0, 30);
  ...
  initial_state.nodes = {root_,       div1_,         button_,
                         div2_,       check_box1_,   check_box2_,
...
TEST_F(AXRangeTest, LeafTextRangeIteration) {
  ...
  auto TestRangeIterator =
      [&expected_ranges](const TestPositionRange& test_range) {
        ...
        for (TestPositionRange anchor_range : test_range) {
          EXPECT_TRUE(anchor_range.IsLeafTextRange());
          actual_ranges.emplace_back(std::move(anchor_range));
        }
  ...
  expected_ranges.emplace_back(line_break_start->Clone(),
...
TEST_F(AXRangeTest, GetTextWithWholeObjects) {
  ...
  EXPECT_EQ(TEXT_FIELD, static_text_range.GetText());
  TestPositionRange static_text_range_backward(std::move(end),
                                               std::move(start));
  EXPECT_EQ(TEXT_FIELD, static_text_range_backward.GetText());
...
TEST_F(AXRangeTest, GetScreenRects) {
  ...
  // |[Button][Checkbox 1][Checkbox 2]L|i|n|e| |1|\n|L|i|n|e| |2|
  // |----------------------------------------------------------|
  TestPositionRange entire_test_range(button->Clone(), line2_end->Clone());
