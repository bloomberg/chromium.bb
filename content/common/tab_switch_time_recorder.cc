...
const char* GetHistogramSuffix(bool has_saved_frames) {
  return has_saved_frames ? "WithSavedFrames" : "NoSavedFrames";
}
...
void TabSwitchTimeRecorder::TabWasHidden() {
  if (!tab_switch_start_time_.is_null()) {
...
void TabSwitchTimeRecorder::RecordHistogramsAndTraceEvents(
  ...
  const auto tab_switch_duration = feedback.timestamp - tab_switch_start_time_;

  // Record trace events.
  TRACE_EVENT_ASYNC_BEGIN_WITH_TIMESTAMP0(
      "latency", "TabSwitching::Latency",
      TRACE_ID_LOCAL(g_num_trace_events_in_process), tab_switch_start_time_);
  ...
  base::UmaHistogramEnumeration(std::string("Browser.Tabs.TabSwitchResult.") +
  ...
  switch (tab_switch_result) {
    case TabSwitchResult::kSuccess: {
      base::UmaHistogramTimes(std::string("Browser.Tabs.TotalSwitchDuration.") +
                                  GetHistogramSuffix(has_saved_frames_),
                              tab_switch_duration);
      break;
    }
  ...
  tab_switch_start_time_ = base::TimeTicks();
