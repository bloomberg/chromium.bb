...
void RasterDecoderImpl::DoBeginRasterCHROMIUM(
  ...
  WrappedSkImage* sk_image_texture = nullptr;
  if (group_->gpu_preferences().enable_raster_to_sk_image) {
    sk_image_texture = WrappedSkImage::CheckedCast(
        group_->mailbox_manager()->ConsumeTexture(mailbox));
  } else {
    texture = gles2::Texture::CheckedCast(
        group_->mailbox_manager()->ConsumeTexture(mailbox));
  }
  if (!texture && !sk_image_texture) {
  ...
  if (final_msaa_count >
    ...
    final_msaa_count = 0;
  if (sk_image_texture) {
    sk_surface_ = sk_image_texture->GetSkSurface(final_msaa_count,
                                                 sk_color_type, surface_props);
  } else {
    GrBackendTexture gr_texture;
    if (!GetGrBackendTexture(gl_version_info(), "glBeginRasterCHROMIUM",
                             GetErrorState(), *texture, color_type,
                             &gr_texture)) {
      return;
    }
  ...
  if (sk_image_texture) {
    if (!sk_image_texture->cleared()) {
      raster_canvas_->drawColor(sk_color);
      sk_image_texture->SetCleared();
    }
  } else {
