...
void RasterDecoderImpl::DoCopySubTextureINTERNAL(
  ...
  DCHECK(dest_target);
  GLint dest_level = 0;
  gfx::Size dest_size = source_shared_image->size();
  ...
  if (gles2::TextureManager::CombineAdjacentRects(
    ...
    cleared_rect = gfx::Rect(dest_size);
  }
  ...
  if (image) {
    ...
    if (image_state == gles2::Texture::UNBOUND) {
      ...
      if (image->BindTexImage(source_target)) {
        source_texture->SetLevelImageState(source_target, 0,
                                           gles2::Texture::BOUND);
      } else {
        bool rv = image->CopyTexImage(source_target);
        DCHECK(rv) << "CopyTexImage() failed";
        source_texture->SetLevelImageState(source_target, 0,
                                           gles2::Texture::COPIED);
      }
    }
  }
