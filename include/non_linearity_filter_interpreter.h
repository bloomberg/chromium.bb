...
class NonLinearityFilterInterpreter : public FilterInterpreter {
 ...
 private:
  ...
  Bounds FindBounds(float value, const scoped_array<double>& range,
                           size_t len) const;
  ...
  bool LoadRange(scoped_array<double> &arr, size_t &len, FILE* fd);
