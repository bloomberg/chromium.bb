...
class StorageApi(object):
  """Base class for object that can download and upload files."""

  def __init__(self):
    self._pool = None

  def set_pool(self, pool):
    """Sets WorkerPool that can be used for parallel uploads."""
    self._pool = pool
  ...
  def get_missing_files(self, files):
    ...
    Issues multiple parallel queries via check_missing_files method calls.
    ...
    """
    # TODO(maruel, vadimsh): Reuse self._pool here.
    with threading_utils.ThreadPool(1, 16, 0, prefix='get_missing_files') as tp:
      for batch in self.batch_files_for_check(files):
        tp.add_task(0, self.check_missing_files, batch)
      for missing in itertools.chain.from_iterable(tp.iter_results()):
        yield missing

  def async_push(self, priority, item, expected_size,
                 content_generator, push_urls=None):
    ...
    data = ''.join(content_generator)
    self._pool.add_task(
        priority, self.push, item, expected_size, [data], push_urls)
...
def upload_tree(base_url, indir, infiles, namespace):
  ...
  # the server.
  cpus = max(threading_utils.num_processors(), 2)
  uploaded = []

  with WorkerPool() as upload_pool:
    remote = get_storage_api(base_url, namespace)
    remote.set_pool(upload_pool)

    def zip_and_trigger_push(filename, metadata, push_urls):
      """Read the file, zips it and trigger push to the storage."""
      # TODO(csharp): Fix crbug.com/150823 and enable the touched logic again.
      path = os.path.join(indir, filename)
      content_generator = zip_compress(file_read(path, ZIPPED_FILE_CHUNK),
          get_zip_compression_level(path))
      if metadata.get('priority', '1') == '0':
        priority = WorkerPool.HIGH
      else:
        priority = WorkerPool.MED
      return remote.async_push(
          priority, metadata['h'], UNKNOWN_FILE_SIZE,
          content_generator, push_urls)

    with threading_utils.ThreadPool(2, cpus, 0, 'zip') as zip_pool:
      for filename, metadata, push_urls in remote.get_missing_files(infiles):
        zip_pool.add_task(0, zip_and_trigger_push,
            filename, metadata, push_urls)
        uploaded.append((filename, metadata))
