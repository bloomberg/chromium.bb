...
def ChromeLibrary(env, target, source, *args, **kw):
  source = compilable_files(env, source)
  return env.Library('$LIB_DIR/' + str(target), source, *args, **kw)

def ChromeLoadableModule(env, target, source, *args, **kw):
  source = compilable_files(env, source)
  return env.LoadableModule(target, source, *args, **kw)

def ChromeStaticLibrary(env, target, source, *args, **kw):
  source = compilable_files(env, source)
  return env.StaticLibrary('$LIB_DIR/' + str(target), source, *args, **kw)
...
def ChromeObject(env, *args, **kw):
  return env.Object(target, source, *args, **kw)
