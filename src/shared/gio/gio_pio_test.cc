...
static int OpenGioPioFile(struct GioPio *gio, const char* name, int oflag) {
  int fd = OPEN(name, oflag, 0666);
  EXPECT_NE(-1, fd);
  return GioPioCtor(gio, fd);
}


TEST(GioPioTest, ReadTest) {
  struct GioPio my_file;
  int ret_code;

  ret_code = OpenGioPioFile(&my_file, g_premade_textfile, O_RDONLY);
  EXPECT_EQ(1, ret_code);
  GioReadTestWithOffset(&my_file.base, 'A');
  GioCloseTest(&my_file.base);

  ret_code = OpenGioPioFile(&my_file, g_premade_binfile, O_RDONLY | _O_BINARY);
  EXPECT_EQ(1, ret_code);
...
void MakeTempFile(struct GioPio* my_file) {
  int ret_code;

  ret_code = OpenGioPioFile(my_file, g_temp_file, O_RDWR | O_CREAT | _O_BINARY);
  EXPECT_EQ(1, ret_code);
}

void CloseAndCleanTempFile(struct GioPio* gf) {
