...
void PpbFileSystemDevRpcServer::PPB_FileSystem_Dev_Create(
    NaClSrpcRpc* rpc,
  ...
  *resource = PPBFileSystemInterface()->Create(
      instance,
      static_cast<PP_FileSystemType_Dev>(file_system_type));
  DebugPrintf("PPB_FileSystem_Dev::Create: resource=%"NACL_PRIu32"\n",
              *resource);

  rpc->result = NACL_SRPC_RESULT_OK;
}

void PpbFileSystemDevRpcServer::PPB_FileSystem_Dev_IsFileSystem(
      NaClSrpcRpc* rpc,
      NaClSrpcClosure* done,
      PP_Resource resource,
      int32_t* success) {
...
void PpbFileSystemDevRpcServer::PPB_FileSystem_Dev_Open(
    NaClSrpcRpc* rpc,
    NaClSrpcClosure* done,
...
void PpbFileSystemDevRpcServer::PPB_FileSystem_Dev_GetType(
    NaClSrpcRpc* rpc,
    NaClSrpcClosure* done,
    PP_Resource file_system,
    int32_t* type) {
  ...
  *type = PPBFileSystemInterface()->GetType(file_system);
  ...
  DebugPrintf("PPB_FileSystem_Dev::GetType: type=%"NACL_PRId32"\n", *type);
