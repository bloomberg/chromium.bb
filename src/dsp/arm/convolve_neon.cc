...
int16x8_t SumTaps(const uint8x8_t* const src, const uint8x8_t* const taps) {
  uint16x8_t sum;
  if (filter_index == 0) {
    // 6 taps. + - + + - +
...
template <int filter_index, bool negative_outside_taps = false>
uint16x8_t SumTaps8To16(const uint8x8_t* const src, const uint8x8_t taps[8]) {
...
LIBGAV1_ALWAYS_INLINE void DoHorizontalPass(
  ...
  if (filter_index == 2) {  // 8 tap.
    FilterHorizontal<8, 8, 2, true, is_2d, is_8bit>(
        src, src_stride, dst, dst_stride, width, height, v_tap);
  } else if (filter_index == 1) {  // 6 tap.
    // Check if outside taps are positive.
    if ((filter_id == 1) | (filter_id == 15)) {
      FilterHorizontal<6, 8, 1, false, is_2d, is_8bit>(
          src, src_stride, dst, dst_stride, width, height, v_tap);
    } else {
      FilterHorizontal<6, 8, 1, true, is_2d, is_8bit>(
          src, src_stride, dst, dst_stride, width, height, v_tap);
    }
  } else if (filter_index == 0) {  // 6 tap.
    FilterHorizontal<6, 8, 0, true, is_2d, is_8bit>(
        src, src_stride, dst, dst_stride, width, height, v_tap);
  } else if (filter_index == 4) {  // 4 tap.
    FilterHorizontal<4, 8, 4, true, is_2d, is_8bit>(
        src, src_stride, dst, dst_stride, width, height, v_tap);
  } else if (filter_index == 5) {  // 4 tap.
    FilterHorizontal<4, 8, 5, true, is_2d, is_8bit>(
        src, src_stride, dst, dst_stride, width, height, v_tap);
  } else {  // 2 tap.
    FilterHorizontal<2, 8, 3, true, is_2d, is_8bit>(
        src, src_stride, dst, dst_stride, width, height, v_tap);
  }
}
...
void VerticalPass2xH(const void* const src, const ptrdiff_t src_stride,
  ...
  // This function does not handle the single pass non-compound convolve.
  // TODO(johannkoenig): Specialize the 2D and compound versions.
  static_assert(is_2d || is_compound, "");
...
void Convolve2D_NEON(const void* const reference,
  ...
  DoHorizontalPass<true>(src, src_stride, intermediate_result, width, width,
                         intermediate_height, subpixel_x, horiz_filter_index);
  ...
  if (width >= 4) {
    ...
    VerticalPass2xH(intermediate_result, width, dest, dest_stride, height, 0,
                    vert_filter_index, vertical_taps, subpixel_y);
  }
}
...
void ConvolveHorizontal_NEON(const void* const reference,
  ...
  DoHorizontalPass<false, true>(src, reference_stride, dest, pred_stride, width,
  ...
  do {
    ...
    do {
      ...
      const int16x8_t sums =
          SumTaps<filter_index, negative_outside_taps>(srcs, taps);
    ...
    do {
      ...
      if (is_compound) {
        const uint16x8_t sums =
            SumTaps8To16<filter_index, negative_outside_taps>(srcs, taps);
...
void ConvolveCompound2D_NEON(const void* const reference,
  ...
  if (width >= 4) {
    ...
    VerticalPass2xH</*is_2d=*/true, /*is_compound=*/true>(
        intermediate_result, width, dest, pred_stride, height,
