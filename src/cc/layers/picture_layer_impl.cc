...
bool PictureLayerImpl::ShouldAdjustRasterScale(float last_scale_aspect_ratio) const {
  ...
  if (draw_properties().screen_space_transform_is_animating ||
      (raster_source_scale_ == ideal_source_scale_ &&
       last_scale_aspect_ratio == scale_aspect_ratio_)) {
    return false;
  }
...
void PictureLayerImpl::RecalculateRasterScales() {
  ...
  gfx::Size raster_bounds =
      gfx::ScaleToCeiledSize(raster_source_->GetSize(),
        raster_contents_scale_, raster_contents_scale_ * scale_aspect_ratio_);
...
void PictureLayerImpl::AdjustRasterScaleForTransformAnimation(
  ...
  gfx::Size bounds_at_maximum_scale =
      gfx::ScaleToCeiledSize(raster_source_->GetSize(), raster_contents_scale_, raster_contents_scale_ * scale_aspect_ratio_);
...
bool PictureLayerImpl::CalculateRasterTranslation(
  ...
  auto is_raster_scale_y = [this](float scale) -> bool {
    return std::abs(scale - raster_contents_scale_ * scale_aspect_ratio_) <= kScaleErrorThreshold;
  };
  if (!is_raster_scale(screen_transform.matrix().getFloat(0, 0)) ||
      !is_raster_scale_y(screen_transform.matrix().getFloat(1, 1)) ||
      !is_raster_scale(draw_transform.matrix().getFloat(0, 0)) ||
      !is_raster_scale_y(draw_transform.matrix().getFloat(1, 1))) {
    return false;
  }
