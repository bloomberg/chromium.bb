  ...
  if (is_compound) {
    ...
    const auto global_motion_type =
        block.tile.frame_header()
            ...
            .type;
    if (IsGlobalMvBlock(mv_bp.is_global_mv_block, global_motion_type)) {
      candidate_mv.mv[0] = prediction_parameters.global_mv[0];
    } else {
...
void AddReferenceMvCandidate(const Tile::Block& block,
  ...
  if (is_compound) {
    if (mv_bp.reference_frame[0] == block.bp->reference_frame[0] &&
      ...
      SearchStack</*is_compound=*/true>(block, mv_bp, 0, weight, found_new_mv,
  ...
  for (int i = 0; i < 2; ++i) {
    if (mv_bp.reference_frame[i] == block.bp->reference_frame[0]) {
      SearchStack</*is_compound=*/false>(block, mv_bp, i, weight, found_new_mv,
                                         found_match, num_mv_found);
    }
  }
}
  ...
  if (is_compound) {
    ...
    *num_mv_found = 2;
    ...
    for (int i = *num_mv_found; i < 2; ++i) {
      ref_mv_stack[i].mv[0] = prediction_parameters.global_mv[0];
      prediction_parameters.SetWeightIndexStackEntry(i, 0);
    }
  }
}
