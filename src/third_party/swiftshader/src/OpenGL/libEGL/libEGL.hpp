...
private:
	LibEGLexports *loadExports()
	{
		if(!loadLibraryAttempted && !libEGL)
		{
			#if defined(_WIN32)
				#if defined(__LP64__)
					const char *libEGL_lib[] = {"libswiftshader_libEGL.dll", SWIFT_EGL_LIBRARY_NAME, "lib64EGL_translator.dll"};
				#else
					const char *libEGL_lib[] = {"libswiftshader_libEGL.dll", SWIFT_EGL_LIBRARY_NAME, "libEGL_translator.dll"};
				#endif
