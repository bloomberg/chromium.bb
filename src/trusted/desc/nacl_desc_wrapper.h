...
namespace nacl {
// Forward declaration for the C++ wrapper for a NaClDesc.
class DescWrapper;
...
// that state and allows creation of wrappers for the general NaClDescs.
class DescWrapperFactory {
  friend class DescWrapper;

 public:
  DescWrapperFactory() : effp_(NULL) {
    sockets_[0] = NULL;
    sockets_[1] = NULL;
  }
  ~DescWrapperFactory();

  // Set up the factory state.  Returns true on success.
  bool Init();
...
class DescWrapper {
 public:
  ...
  DescWrapper(DescWrapperFactory* factory, struct NaClDesc* desc)
      : factory_(factory), desc_(desc) {
  }
  ~DescWrapper();
  ...
  // (-4K, -1] as NaCl ABI errno on failure.
  ...
  int SendMsg(MsgHeader* dgram, int flags);
