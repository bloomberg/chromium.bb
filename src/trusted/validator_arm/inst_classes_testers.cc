...
ApplySanityChecks(Instruction inst,
  ...
  EXPECT_EQ(expected_decoder.imm4_.value(inst), inst.bits(19, 16));
  EXPECT_EQ(expected_decoder.imm12_.value(inst), inst.bits(11, 0));
  ...
  EXPECT_NE(expected_decoder.d_.number(inst), (uint32_t) 15)
...
ApplySanityChecks(Instruction inst,
  ...
  if ((expected_decoder.d_.reg(inst) == kRegisterPc) &&
      expected_decoder.flags_.is_updated(inst) &&
      (&ExpectedDecoder() != &decoder))
    return;
...
ApplySanityChecks(Instruction inst,
  ...
  // Check Registers and flags used in DataProc.
  EXPECT_EQ(expected_decoder.d_.number(inst), inst.bits(15, 12));
  EXPECT_EQ(expected_decoder.m_.number(inst), inst.bits(11, 8));
  EXPECT_EQ(expected_decoder.n_.number(inst), inst.bits(3, 0));
  EXPECT_EQ(expected_decoder.flags_.is_updated(inst), inst.bit(20));
  if (expected_decoder.flags_.is_updated(inst)) {
    EXPECT_EQ(expected_decoder.flags_.reg_if_updated(inst), kRegisterFlags);
  } else {
...
ApplySanityChecks(Instruction inst,
  ...
  EXPECT_NE(expected_decoder.d_.number(inst), (uint32_t) 15)
      << "Expected Unpredictable for " << InstContents();
  EXPECT_NE(expected_decoder.m_.number(inst), (uint32_t) 15)
      << "Expected Unpredictable for " << InstContents();
  EXPECT_NE(expected_decoder.n_.number(inst), (uint32_t) 15)
      << "Expected Unpredictable for " << InstContents();
}
...
ApplySanityChecks(Instruction inst,
  ...
  // Check that immediate value is computed correctly.
  EXPECT_EQ(expected_decoder.imm_.value(inst), inst.bits(11, 7));
  EXPECT_EQ(expected_decoder.shift_type_.value(inst), inst.bits(6, 5));
...
ApplySanityChecks(Instruction inst,
  ...
  if ((0 == expected_decoder.imm_.value(inst)) &&
      (&ExpectedDecoder() != &decoder))
    return;
...
ApplySanityChecks(Instruction inst,
  ...
  EXPECT_NE(expected_decoder.d_.number(inst), (uint32_t) 15)
      << "Expected Unpredictable for " << InstContents();
  EXPECT_NE(expected_decoder.s_.number(inst), (uint32_t) 15)
      << "Expected Unpredictable for " << InstContents();
  EXPECT_NE(expected_decoder.m_.number(inst), (uint32_t) 15)
      << "Expected Unpredictable for " << InstContents();
}
...
ApplySanityChecks(Instruction inst,
  ...
  if ((expected_decoder.n_.reg(inst) == kRegisterStack) &&
      (&ExpectedDecoder() != &decoder))
    return;
  Binary3RegisterImmedShiftedOpTesterNotRdIsPcAndS::
