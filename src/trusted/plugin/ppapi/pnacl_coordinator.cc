...
void WINAPI DoTranslateThread(void* arg) {
  ...
  const char* llc_args_x8632[] = { "-march=x86",
                                   ...
                                   "-asm-verbose=false",
                                   "-filetype=obj" };
  const char* llc_args_x8664[] = { "-march=arm",
                                   "-mcpu=cortex-a8",
                                   "-mtriple=x86_64-none-linux-gnu",
                                   "-asm-verbose=false",
                                   "-filetype=obj" };
  const char* llc_args_arm[] = { "-march=x86-64",
                                 "-mcpu=core2",
...
//////////////////////////////////////////////////////////////////////
// Helper functions for loading native libs and linker script.
// Done here to avoid hacking on the manifest parser further...

namespace {

nacl::string DashFreeISA(const nacl::string& sandbox_isa) {
  if (sandbox_isa.compare("x86-32") == 0) {
    return "x8632";
  } else if (sandbox_isa.compare("x86-64") == 0) {
    return "x8664";
    ...
    PLUGIN_PRINTF(("DashFreeISA given unknown ISA %s\n", sandbox_isa.c_str()));
    return "___UNKNOWN_ARCH___";
  }
}
...
nacl::string GetLinkerScriptName(const nacl::string& sandbox_isa) {
  nacl::string dash_free_isa = DashFreeISA(sandbox_isa);
  nacl::string base_dir = "pnacl_support/" + dash_free_isa;
  return base_dir + "/ld_script_" + dash_free_isa + "_untrusted";
}

string_vector LinkResources(const nacl::string& sandbox_isa,
                            bool withGenerated,
                            bool withLinkerScript) {
  ...
  //  results.push_back(base_dir + "/crt1.o");
  if (withGenerated) {
  ...
  if (withLinkerScript) {
    results.push_back(GetLinkerScriptName(sandbox_isa));
  }
  return results;
}
...
void WINAPI DoLinkThread(void* arg) {
  ...
  flags.push_back("-T");
  flags.push_back(GetLinkerScriptName(sandbox_isa));
  ...
  string_vector files = LinkResources(sandbox_isa, true, true);
  for (string_vector::iterator i = files.begin(), e = files.end();
    ...
    // Add as argument, but skip linker script (already in the arguments).
    if (!(i->find("ld_script") != nacl::string::npos)) {
      if (!PnaclSrpcLib::InvokeSrpcMethodNoOutput(p->browser,
                                                  ld_subprocess,
                                                  "AddArg",
                                                  "C",
...
void PnaclCoordinator::BitcodeToNative(
  ...
  string_vector link_resources = LinkResources(GetSandboxISA(),
                                               false,
                                               true);
