...
void PrintRenderFrameHelper::PrintHeaderAndFooter(
  ...
  options.Set("headerText", source_frame.GetDocument().bbHeaderText().Utf8());
  options.Set("footerText", source_frame.GetDocument().bbFooterText().Utf8());
  options.Set("printPageNumbers", source_frame.GetDocument().bbPrintPageNumbers());
#endif
...
std::vector<char> PrintRenderFrameHelper::PrintToPDF(
  ...
  if (CalculateNumberOfPages(localframe, blink::WebNode(),
    ...
    // defined below
    std::vector<uint32_t> printed_pages = PageNumber::GetPages(pagesParams.pages, page_count);
