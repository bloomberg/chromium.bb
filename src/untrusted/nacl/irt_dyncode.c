...
static int nacl_irt_dyncode_create(void *dest, const void *src, size_t size) {
  return NACL_GC_WRAP_SYSCALL(-NACL_SYSCALL(dyncode_create)(dest, src, size));
}

static int nacl_irt_dyncode_modify(void *dest, const void *src, size_t size) {
  return NACL_GC_WRAP_SYSCALL(-NACL_SYSCALL(dyncode_modify)(dest, src, size));
}

static int nacl_irt_dyncode_delete(void *dest, size_t size) {
  return NACL_GC_WRAP_SYSCALL(-NACL_SYSCALL(dyncode_delete)(dest, size));
}
