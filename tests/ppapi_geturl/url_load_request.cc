...
bool UrlLoadRequest::Init(std::string url,
  ...
  if ((PP_OK != open_err) && (PP_ERROR_WOULDBLOCK != open_err)) {
    ...
    printf("--- UrlLoadRequest::Init() : "
           "error %"NACL_PRId32" [%s] calling PPB_URLLoader_Dev::Open\n",
           open_err,
           err_text.c_str());
    *error = "error calling PPB_URLLoader_Dev::Open";
    return false;
  }
  return true;
}
...
bool UrlLoadRequest::GetRequiredInterfaces(std::string* error) {
  Module* module = Module::Get();
  req_info_interface_ = static_cast<const PPB_URLRequestInfo_Dev*>(
      module->GetBrowserInterface(PPB_URLREQUESTINFO_DEV_INTERFACE));
  if (NULL == req_info_interface_) {
    std::string msg = "Failed to get interface '";
    msg.append(PPB_URLREQUESTINFO_DEV_INTERFACE);
  ...
  if (0 == url_request_) {
    ...
    snprintf(tmp,
             sizeof(tmp) - 1,
             PPB_URLREQUESTINFO_DEV_INTERFACE "::Create() failed");
    tmp[sizeof(tmp) - 1] = 0;
    *error = tmp;
    return false;
  }

  loader_interface_ = static_cast<const PPB_URLLoader_Dev*>(
          module->GetBrowserInterface(PPB_URLLOADER_DEV_INTERFACE));

  if (NULL == loader_interface_) {
    std::string msg = "Failed to get interface '";
    msg.append(PPB_URLLOADER_DEV_INTERFACE);
    msg.append("' from browser.");
    *error = msg;
    return false;
  }
  ...
  if (0 == loader_) {
    *error = PPB_URLLOADER_DEV_INTERFACE "::Create() failed";
    return false;
  }
  return true;
}
...
void UrlLoadRequest::ReadBody() {
  ...
  if ((PP_OK != read_result) && (PP_ERROR_WOULDBLOCK != read_result)) {
    std::string error = "PPB_URLLoader_Dev::ReadResponseBody error:";
