...
template<typename T> struct Helper { static int Bits(); };
template<> INLINE int Helper<uint8_t>::Bits() { return 8; }
template<> INLINE int Helper<uint16_t>::Bits() { return 16; }
template<> INLINE int Helper<uint32_t>::Bits() { return 32; }
template<> INLINE int Helper<uint64_t>::Bits() { return 64; }
...
TEST_F(BitsTest, CountTrailingZeroes32) {
  TestAllInputsInRange(&nacl::CountTrailingZeroes<uint32_t>,
                       &BitTest::CountTrailingZeroes<uint32_t>,
                       (uint32_t)0,
                       (uint32_t)std::numeric_limits<uint16_t>::max() + kExtra);
  TestAllInputsInRange(&nacl::CountTrailingZeroes<uint32_t>,
                       &BitTest::CountTrailingZeroes<uint32_t>,
...
TEST_F(BitsTest, CountLeadingZeroes32) {
  TestAllInputsInRange(&nacl::CountLeadingZeroes<uint32_t>,
                       &BitTest::CountLeadingZeroes<uint32_t>,
                       (uint32_t)0,
                       (uint32_t)std::numeric_limits<uint16_t>::max() + kExtra);
  TestAllInputsInRange(&nacl::CountLeadingZeroes<uint32_t>,
                       &BitTest::CountLeadingZeroes<uint32_t>,
