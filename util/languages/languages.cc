...
static const LanguageInfo* LanguageInfoTable() {
  static const LanguageInfo language_info_table[] = {
    { "ENGLISH",             "en", "eng", NULL},
    { "DANISH",              "da", "dan", NULL},
    { "DUTCH",               "nl", "dut", NULL},
    { "FINNISH",             "fi", "fin", NULL},
  ...
  COMPILE_ASSERT(arraysize(language_info_table) == NUM_LANGUAGES + 1,
                 language_info_table_has_incorrect_length);

  return language_info_table;
}

// LANGUAGE NAMES

const char* default_language_name() {
  return LanguageInfoTable()[ENGLISH].language_name_;
}

const char *invalid_language_name() {
  static const char* const invalid_language_name = "invalid_language";
  return invalid_language_name;
}

const char* LanguageName(Language lang) {
  return IsValidLanguage(lang)
      ? LanguageInfoTable()[lang].language_name_
...
const char * LanguageCode(Language lang) {
  ...
  const LanguageInfo& info = LanguageInfoTable()[lang];
...
const char* default_language_code() {
  return LanguageInfoTable()[ENGLISH].language_code_639_1_;
}

const char* LanguageCodeISO639_1(Language lang) {
  if (! IsValidLanguage(lang))
    return invalid_language_code();
  if (const char* code = LanguageInfoTable()[lang].language_code_639_1_)
    return code;
  return invalid_language_code();
}
...
const char* LanguageCodeISO639_2(Language lang) {
  if (! IsValidLanguage(lang))
    return invalid_language_code();
  if (const char* code = LanguageInfoTable()[lang].language_code_639_2_)
    return code;
  return invalid_language_code();
}
...
bool LanguageFromCode(const char* lang_code, Language *language) {
  *language = UNKNOWN_LANGUAGE;
  ...
  for ( int i = 0 ; i < kNumLanguages ; i++ ) {
    const LanguageInfo& info = LanguageInfoTable()[i];
    if ((info.language_code_639_1_ &&
