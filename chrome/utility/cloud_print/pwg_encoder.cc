...
template void encodePixelToRGB<RGBA8>(const void* pixel, std::string* output);
template void encodePixelToRGB<BGRA8>(const void* pixel, std::string* output);
template void encodePixelToMonochrome<RGBA8>(const void* pixel,
                                             std::string* output);
template void encodePixelToMonochrome<BGRA8>(const void* pixel,
                                             std::string* output);
...
void PwgEncoder::EncodePageHeader(const BitmapImage* image,
  ...
  base::WriteBigEndian<uint32>(header + kHeaderCupsWidth,
                               image->size().width());
...
bool PwgEncoder::EncodePage(const BitmapImage* image,
  ...
  switch (image->colorspace()) {
    case BitmapImage::RGBA:
      pixel_encoder = monochrome ? &encodePixelToMonochrome<RGBA8>
                                 : &encodePixelToRGB<RGBA8>;
      break;
    case BitmapImage::BGRA:
      pixel_encoder = monochrome ? &encodePixelToMonochrome<BGRA8>
                                 : &encodePixelToRGB<BGRA8>;
      break;
    default:
  ...
  while (row_number < image->size().height()) {
    ...
    if (!pwg_header_info.flipx) {
      EncodeRow(pos, row_end, pixel_encoder, output);
    } else {
      // We reverse the iterators.
      EncodeRow(std::reverse_iterator<const uint32*>(row_end),
