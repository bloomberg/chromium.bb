...
import {ColorModeRestriction, Destination, DestinationConnectionStatus, DestinationOrigin, DestinationType, DestinationStore, DuplexModeRestriction, InvitationStore, NativeLayer} from 'chrome://print/print_preview.js';
  ...
  suite(destination_search_test_chromeos.suiteName, function() {
    ...
    test(assert(
      ...
      const response = {
        printerId: destId,
        capabilities:
            getCddTemplate(destId).capabilities,
        success: true,
      };
    ...
    test(assert(destination_search_test_chromeos.TestNames.ReceiveFailedSetup),
        function() {
      const originalDestination = destinationStore.selectedDestination;
      const destId = '00112233DEADBEEF';
      const response = {
    ...
    test(assert(destination_search_test_chromeos.TestNames.CloudKioskPrinter),
        function() {
      ...
      const cloudDest = new Destination(
          ...
          DestinationOrigin.DEVICE, 'displayName',
          DestinationConnectionStatus.ONLINE);
      cloudDest.capabilities =
          getCddTemplate(printerId, 'displayName')
              .capabilities;
