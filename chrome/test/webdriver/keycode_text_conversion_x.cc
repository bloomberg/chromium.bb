...
// Gets the X modifier mask (Mod1Mask through Mod5Mask) for the given automation
...
bool GetXModifierMask(Display* display, int modifier, int* x_modifier) {
  ...
  for (int mod_index = 0; mod_index <= 8; ++mod_index) {
    for (int key_index = 0; key_index < max_mod_keys; ++key_index) {
      ...
      else if (modifier == automation::kNumLockKeyMask)
...
std::string ConvertKeyCodeToText(ui::KeyboardCode key_code, int modifiers) {
  ...
  if (modifiers & automation::kAltKeyMask &&
      GetXModifierMask(display, automation::kAltKeyMask, &x_modifier)) {
    key_event->state |= x_modifier;
  }
  if (modifiers & automation::kMetaKeyMask &&
      GetXModifierMask(display, automation::kMetaKeyMask, &x_modifier)) {
    key_event->state |= x_modifier;
  }
  if (modifiers & automation::kNumLockKeyMask &&
      GetXModifierMask(display, automation::kNumLockKeyMask, &x_modifier)) {
    key_event->state |= x_modifier;
  }
...
bool ConvertCharToKeyCode(
  ...
  for (size_t i = 0; i < arraysize(kKeyCodeToXKeyCode); ++i) {
    ...
    test_modifiers = automation::kShiftKeyMask;
    if (ConvertKeyCodeToText(test_code, test_modifiers) == key_string) {
      found = true;
      break;
    }
