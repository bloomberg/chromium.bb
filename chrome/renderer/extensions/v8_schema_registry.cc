...
void V8SchemaRegistry::InitializeContext() {
  // The context is lazily intialized, as creating a Context and disposing it
  // without using it triggers an assert.
  if (!context_.get())
    context_.reset(new v8::Persistent<v8::Context>(v8::Context::New()));
}
