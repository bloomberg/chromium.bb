...
MessageLoop* ExtensionBindingsContext::render_thread_message_loop_ = NULL;
...
void ExtensionBindingsContext::HandleV8ContextCreated(
  ...
  g_contexts.Pointer()->push_back(instance);
...
void ExtensionBindingsContext::HandleV8ContextReleased(
  ...
  for (ContextList::iterator iter = g_contexts.Pointer()->begin();
    ...
    ExtensionBindingsContext* bindings_context = *iter;
    if (bindings_context->v8_context() == context) {
      bindings_context->web_frame_ = NULL;
      g_contexts.Pointer()->erase(iter);

      // CallChromeHiddenMethod() can run JavaScript that ends up closing the
      // frame, and therefore calling this. We don't want anyone who calls
      // CallChromeHiddenMethod() to have to worry about subsequently holding a
      // dead object.
      GetRenderThreadMessageLoop()->DeleteSoon(FROM_HERE, bindings_context);
...
void ExtensionBindingsContext::SetRenderThreadMessageLoop(MessageLoop* loop) {
  render_thread_message_loop_ = loop;
}

MessageLoop* ExtensionBindingsContext::GetRenderThreadMessageLoop() {
  return render_thread_message_loop_ ?
