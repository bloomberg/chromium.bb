...
class GroupPolicyTest : public testing::Test {
 protected:
  ...
  virtual void SetUp() {
    ...
    RegKey hkcu = RegKey(HKEY_CURRENT_USER, policy_key.c_str(),
                         KEY_WRITE | KEY_READ);
    RegKey hklm = RegKey(HKEY_LOCAL_MACHINE, policy_key.c_str(),
                         KEY_WRITE | KEY_READ);
    can_set_hkcu_ = hkcu.Valid();
    can_set_hklm_ = hklm.Valid();
  ...
  virtual void TearDown() {
    ...
    if (can_set_hklm_) {
      RegKey hklm_list = RegKey(HKEY_LOCAL_MACHINE,
                                policy_key.c_str(),
                                KEY_WRITE);
      EXPECT_TRUE(hklm_list.DeleteKey(L"ChromeFrameDomainWhiteList"));
    }
  }
};
...
TEST_F(GroupPolicyTest, TestGetGroupPolicyString) {
  ...
  bool found = false;
  if (can_set_hklm_ || can_set_hkcu_) {
    ...
    if (can_set_hklm_)
      ...
      EXPECT_EQ(0, setting.compare(L"www.google.com/hkcu"));
  }
}
...
TEST_F(GroupPolicyTest, TestGetStringList) {
  ...
  if (can_set_hklm_ || can_set_hkcu_) {
    ...
    EXPECT_TRUE(found);
    EXPECT_TRUE(SUCCEEDED(hr));
    EXPECT_EQ((can_set_hklm_ ? 6 : 3), mixed_list->GetSize());
