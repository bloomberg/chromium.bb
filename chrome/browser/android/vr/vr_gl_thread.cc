...
void VrGLThread::SetFullscreen(bool enabled) {
  ...
  task_runner()->PostTask(
      FROM_HERE,
      base::BindOnce(&BrowserUiInterface::SetFullscreen, browser_ui_, enabled));
}

void VrGLThread::SetIncognito(bool incognito) {
  DCHECK(OnMainThread());
  task_runner()->PostTask(
      FROM_HERE, base::BindOnce(&BrowserUiInterface::SetIncognito, browser_ui_,
                                incognito));
}
...
void VrGLThread::SetLoading(bool loading) {
  ...
  task_runner()->PostTask(
      FROM_HERE,
      base::BindOnce(&BrowserUiInterface::SetLoading, browser_ui_, loading));
}
...
void VrGLThread::SetToolbarState(const ToolbarState& state) {
  ...
  task_runner()->PostTask(
      FROM_HERE,
      base::BindOnce(&BrowserUiInterface::SetToolbarState, browser_ui_, state));
}

void VrGLThread::SetWebVrMode(bool enabled, bool show_toast) {
  DCHECK(OnMainThread());
  task_runner()->PostTask(
      FROM_HERE, base::BindOnce(&BrowserUiInterface::SetWebVrMode, browser_ui_,
                                enabled, show_toast));
}
...
void VrGLThread::SetIsExiting() {
  ...
  task_runner()->PostTask(
      FROM_HERE,
      base::BindOnce(&BrowserUiInterface::SetIsExiting, browser_ui_));
}
...
void VrGLThread::SetExitVrPromptEnabled(bool enabled,
  ...
  DCHECK(OnMainThread());
  task_runner()->PostTask(
      FROM_HERE, base::BindOnce(&BrowserUiInterface::SetExitVrPromptEnabled,
...
void VrGLThread::SetOmniboxSuggestions(
  ...
  task_runner()->PostTask(
      FROM_HERE,
      base::BindOnce(&BrowserUiInterface::SetOmniboxSuggestions, browser_ui_,
...
void VrGLThread::OnAssetsComponentReady() {
  ...
  task_runner()->PostTask(
      FROM_HERE,
      base::BindOnce(&BrowserUiInterface::OnAssetsComponentReady, browser_ui_));
}
...
void VrGLThread::ShowSoftInput(bool show) {
  ...
  task_runner()->PostTask(
      FROM_HERE,
      base::BindOnce(&BrowserUiInterface::ShowSoftInput, browser_ui_, show));
}
...
void VrGLThread::UpdateWebInputSelectionIndices(int selection_start,
  ...
  task_runner()->PostTask(
      FROM_HERE,
      base::BindOnce(&BrowserUiInterface::UpdateWebInputSelectionIndices,
...
void VrGLThread::UpdateWebInputCompositionIndices(int composition_start,
  ...
  task_runner()->PostTask(
      FROM_HERE,
      base::BindOnce(&BrowserUiInterface::UpdateWebInputCompositionIndices,
...
void VrGLThread::UpdateWebInputText(const base::string16& text) {
  DCHECK(OnMainThread());
  task_runner()->PostTask(
      FROM_HERE, base::BindOnce(&BrowserUiInterface::UpdateWebInputText,
                                browser_ui_, text));
}
