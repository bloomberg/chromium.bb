...
class RequirementsCheckerBrowserTest : public ExtensionBrowserTest {
 public:
  ...
  void BlackListGPUFeatures(std::vector<content::GpuFeatureType> features) {
    std::vector<std::string> string_features;
    for (std::vector<content::GpuFeatureType>::iterator it = features.begin();
         it != features.end();
         ++it) {
      string_features.push_back(
          "\"" + gpu_util::GpuFeatureTypeToString(*it) + "\"");
    }
    ...
    static const std::string json_blacklist =
      ...
      "    {\n"
      "      \"id\": 1,\n"
      "      \"blacklist\": [" + JoinString(string_features, ',') + "]\n"
      "    }\n"
      "  ]\n"
      "}";
...
IN_PROC_BROWSER_TEST_F(RequirementsCheckerBrowserTest, DisallowCSS3D) {
  ...
  // Blacklist css3d
  std::vector<content::GpuFeatureType> blacklisted_features;
  blacklisted_features.push_back(
...
IN_PROC_BROWSER_TEST_F(RequirementsCheckerBrowserTest, DisallowWebGL) {
  ...
  // Backlist webgl
  std::vector<content::GpuFeatureType> blacklisted_features;
  blacklisted_features.push_back(content::GPU_FEATURE_TYPE_WEBGL);
  BlackListGPUFeatures(blacklisted_features);
  content::BrowserThread::GetBlockingPool()->FlushForTesting();
