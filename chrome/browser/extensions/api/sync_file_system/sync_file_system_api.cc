  ...
  for (unsigned int i = 0; i < num_expected_results_; i++) {
    ...
    sync_file_system_service->GetFileSyncStatus(
        ...
        Bind(&SyncFileSystemGetFileStatusesFunction::DidGetFileStatus,
             this, file_system_url.path().AsUTF8Unsafe()));
  }
...
void SyncFileSystemGetFileStatusesFunction::DidGetFileStatus(
  ...
  sync_status_codes_[file_path] = sync_status_code;
  file_sync_statuses_[file_path] = FileSyncStatusEnumToExtensionEnum(
      sync_file_status);
  ...
  std::map<std::string, api::sync_file_system::FileStatus>::iterator it;
  for (it = file_sync_statuses_.begin();
       it != file_sync_statuses_.end(); ++it) {
    DictionaryValue* dict = new DictionaryValue();
    status_array->Append(dict);
    std::string file_entry_path = it->first;
    dict->SetString("file_path", file_entry_path);
    dict->SetString("status", ToString(it->second));

    SyncStatusCode file_error = sync_status_codes_.at(file_entry_path);
