...
// Check failed: metric->GetIOCounters(&before), http://crbug.com/46040.
#if defined(OS_MACOSX)
...
TEST(SafeBrowsingDatabase, SKIP_MACOSX(DatabaseInitialIO)) {
...
TEST(SafeBrowsingDatabase, SKIP_MACOSX(DatabaseOldIO)) {
...
TEST(SafeBrowsingDatabase, SKIP_MACOSX(DatabaseOldUpdatesIO)) {
  UpdateDatabase(GetOldSafeBrowsingPath(), FilePath(), GetOldUpdatesPath());
}

// Does a a lot of addel's on very large chunks.
TEST(SafeBrowsingDatabase, SKIP_MACOSX(DatabaseOldLotsofDeletesIO)) {
  std::vector<ChunksInfo> chunks;
  std::vector<SBChunkDelete> deletes;
  SBChunkDelete del;
