...
bool SyncedSessionTracker::LookupTabNodeIds(
    const std::string& session_tag, std::set<size_t>* tab_node_ids) {
  tab_node_ids->clear();
...
SessionTab* SyncedSessionTracker::GetTabImpl(
  ...
  if (iter != synced_tab_map_[session_tag].end()) {
    ...
    if (iter->second.tab_node_id == TabNodePool::kInvalidTabNodeID &&
    ...
    } else if (tab_node_id != TabNodePool::kInvalidTabNodeID) {
      DCHECK_EQ(tab_node_id, iter->second.tab_node_id);
    }
    if (VLOG_IS_ON(1)) {
      std::string title;
      if (tab_ptr->navigations.size() > 0) {
