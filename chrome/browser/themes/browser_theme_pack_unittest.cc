...
TEST_F(BrowserThemePackTest, BuildFromColor_TestColors) {
  constexpr SkColor backgrounds[] = {
      SK_ColorBLACK, SkColorSetRGB(50, 0, 50), SkColorSetRGB(0, 200, 200),
      SkColorSetRGB(125, 125, 125), SkColorSetRGB(0, 133, 133)};
  auto has_readable_contrast = [](SkColor color) {
    return color_utils::GetContrastRatio(
               color, color_utils::GetColorWithMaxContrast(color)) >=
  ...
  for (SkColor color : backgrounds) {
    ...
    SkColor frame_color;
    EXPECT_TRUE(pack->GetColor(TP::COLOR_FRAME, &frame_color));
    EXPECT_TRUE(has_readable_contrast(frame_color));

    SkColor toolbar_color, ntp_background;
    EXPECT_TRUE(pack->GetColor(TP::COLOR_TOOLBAR, &toolbar_color));
