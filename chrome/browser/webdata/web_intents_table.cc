...
// Updates the table by way of renaming the old tables,
// rerunning the Init method, then selecting old values
// into the new tables.
bool WebIntentsTable::MigrateToVersion46AddSchemeColumn() {
  if (!db_->Execute("ALTER TABLE web_intents RENAME TO old_web_intents")) {
    LOG(WARNING) << "Could not backup web_intents table.";
    return false;
  }

  if (!db_->Execute("ALTER TABLE web_intents_defaults"
                    " RENAME TO old_web_intents_defaults")) {
    LOG(WARNING) << "Could not backup web_intents_defaults table.";
    return false;
  }
  ...
  if (!db_->Execute(
    ...
    LOG(WARNING) << "Could not restore entries from backup web_intents table.";
    return false;
  }
  ...
  if (!db_->Execute(
    ...
    LOG(WARNING)
        << "Could not restore entries from backup web_intents_defaults table.";
    return false;
  }
  ...
  if (!db_->Execute("DROP table old_web_intents_defaults")) {
    LOG(WARNING) << "Could not drop backup web_intents_defaults table.";
    return false;
  }
