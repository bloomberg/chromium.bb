...
enum PolicyCombine {
  POLICY_COMBINE_PREFERMACHINE,    // HKLM policy takes priority.
  POLICY_COMBINE_PREFERUSER,       // HKCU policy takes priority.
  POLICY_COMBINE_CONCATENATE,      // Concatenate both policies.
  POLICY_COMBINE_LOGICALOR,        // Logical OR of both keys.
};

// This specifies how the policy setting is stored in the registry.
// Add more as needed.
enum PolicyStorage {
  POLICY_STORAGE_SINGLEVALUE,    // Policy is a single registry value.
  POLICY_STORAGE_CONCATSUBKEYS,  // Policy is set of subkeys of specified key.
};
...
class SettingBase {
 public:
  SettingBase(const wchar_t* regkey,
    ...
    // If storage is POLICY_STORAGE_CONCATSUBKEYS, regvalue should be empty.
    DCHECK((storage != POLICY_STORAGE_CONCATSUBKEYS) ||
           regvalue[0] == L'\0');
  }
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(SettingBase);
};
