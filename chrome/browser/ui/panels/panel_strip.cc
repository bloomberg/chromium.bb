...
void PanelStrip::BringUpOrDownTitlebars(bool bring_up) {
  ...
  if (desktop_bar->IsEnabled(AutoHidingDesktopBar::ALIGN_BOTTOM)) {
    ...
    if (visibility != (bring_up ? AutoHidingDesktopBar::VISIBLE
      ...
      // of time.
      task_delay = kMaxDelayWaitForBottomBarVisibilityChangeMs;
    }
  }
  ...
  if (!bring_up && task_delay == 0) {
    task_delay = kDelayBeforeCollapsingFromTitleOnlyStateMs;
  }
  ...
  MessageLoop::current()->PostDelayedTask(
      ...
      base::TimeDelta::FromMilliseconds(
          PanelManager::AdjustTimeInterval(task_delay)));
}
