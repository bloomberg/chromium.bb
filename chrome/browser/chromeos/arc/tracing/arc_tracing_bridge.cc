...
class ArcTracingDataSource : public tracing::ProducerClient::DataSourceBase {
 ...
 private:
  ...
  // Starts all registered data sources.
  ...
  // Stops all registered data sources. Calls |stop_complete_callback| when all
  // data sources have stopped.
  void StopTracingOnUI(base::OnceClosure stop_complete_callback) {
    ...
    // There may not have been any bridges left in State::kEnabled.
    if (!did_request_stop) {
