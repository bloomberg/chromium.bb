...
UploadJob::MimeBoundaryGenerator::~MimeBoundaryGenerator() {
}

UploadJob::RandomMimeBoundaryGenerator::~RandomMimeBoundaryGenerator() {
}
...
class DataSegment {
 public:
  ...
  // Returns the string that will be assigned to the |name| field in the
  // header. |name| must be unique throughout the multipart message. This is
  // enforced in SetUpMultipart().
  ...
  // Returns the data contained in this |DataSegment|. Ownership
  // is passed.
...
std::string UploadJob::RandomMimeBoundaryGenerator::GenerateBoundary(
    size_t length) const {
  std::string boundary;
  boundary.reserve(length);
  DCHECK_LT(0, (int)length - (int)(sizeof(kMultipartBoundaryPrefix) +
...
void UploadJob::RequestAccessToken() {
  state_ = ACQUIRING_TOKEN;
  access_token_request_ =
      token_service_->StartRequest(account_id_, scope_set_, this);
}

bool UploadJob::SetUpMultipart() {
...
void UploadJob::CreateAndStartURLFetcher(const std::string& access_token) {
...
void UploadJob::StartUpload(const std::string& access_token) {
  if (!SetUpMultipart()) {
...
void UploadJob::OnGetTokenSuccess(const OAuth2TokenService::Request* request,
...
void UploadJob::OnGetTokenFailure(const OAuth2TokenService::Request* request,
...
void UploadJob::OnURLFetchComplete(const net::URLFetcher* source) {
  ...
  if (response_code == net::HTTP_UNAUTHORIZED) {
    ...
    retry_++;
    upload_fetcher_.reset();
    token_service_->InvalidateToken(account_id_, scope_set_, access_token_);
    access_token_.clear();
    RequestAccessToken();
    return;
  }
