...
class TestGithubFileSystem(unittest.TestCase):
  ...
  def testStat(self):
    ...
    dir_stat = StatInfo(FAKE_HASH, {
      'hello.py': StatInfo(FAKE_HASH),
      '__init__.py': StatInfo(FAKE_HASH)
    })

    self.assertEqual(StatInfo(FAKE_HASH), self._gfs.Stat('/README.md'))
    self.assertEqual(StatInfo(FAKE_HASH), self._gfs.Stat('/src/hello.py'))
  ...
  def testCachingFileSystem(self):
    ...
    initial_cgfs_read_one = self._cgfs.ReadSingle('/src/hello.py')

    self.assertEqual(
        initial_cgfs_read_one,
        self._gfs.ReadSingle('/src/hello.py'))
    self.assertEqual(
        initial_cgfs_read_one,
        self._cgfs.ReadSingle('/src/hello.py'))
