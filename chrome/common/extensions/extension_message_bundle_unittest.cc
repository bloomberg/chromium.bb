...
class ExtensionMessageBundleTest : public testing::Test {
 protected:
  ...
  bool IsValidMessageName(std::string name) {
...
TEST_F(ExtensionMessageBundleTest, IsValidNameCheckValidCharacters) {
  EXPECT_TRUE(IsValidMessageName("a__BV_9"));
  EXPECT_TRUE(IsValidMessageName("@@a__BV_9"));
  EXPECT_FALSE(IsValidMessageName("$a__BV_9$"));
  EXPECT_FALSE(IsValidMessageName("a-BV-9"));
  EXPECT_FALSE(IsValidMessageName("a#BV!9"));
  EXPECT_FALSE(IsValidMessageName("a<b"));
}
