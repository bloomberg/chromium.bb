...
const int kFaviconSizeInDip = gfx::kFaviconSize;

// 2x size of the favicon.
const int kFaviconSizeInDip2x = gfx::kFaviconSize * 2;

// 4x size of the favicon.
const int kFaviconSizeInDip4x = gfx::kFaviconSize * 4;
...
bool ParseFaviconPath(const std::string& path,
  ...
  parsed->url = "";
  parsed->size_in_dip = kFaviconSizeInDip;
  ...
  if (HasSubstringAt(path, parsed_index, kLargestParameter)) {
    ...
    if (parsed->size_in_dip != kFaviconSizeInDip4x &&
        parsed->size_in_dip != kFaviconSizeInDip2x) {
