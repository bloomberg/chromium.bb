             ...
             'actions': [
               {
                 ...
                 'action': [
                   ...
                   '--link_flags=--target=i686-unknown-nacl -stdlib=libstdc++ -arch x86-32 --pnacl-allow-translate --pnacl-allow-native -Wt,-force-align-stack -B>(tc_lib_dir_irt32) ^(pnacl_irt_link_flags) ^(link_flags) >(_link_flags)',
                   '--source-list=^(source_list_newlib32)',
                   '--tls-edit=<(PRODUCT_DIR)/tls_edit<(EXECUTABLE_SUFFIX)',
                   '--irt-layout',
                 ],
             ...
             'actions': [
               {
                 ...
                 'action': [
                   ...
                   # X86-32 IRT needs to be callable with an under-aligned stack: see
                   # https://code.google.com/p/nativeclient/issues/detail?id=3935
                   '--compile_flags=--target=i686-unknown-nacl -stdlib=libstdc++ -Wt,-force-align-stack ^(compile_flags) >(_compile_flags) -gline-tables-only ^(pnacl_compile_flags) >(_pnacl_compile_flags)',
                   '--gomadir', '<(gomadir)',
                   '--defines=^(defines) >(_defines)',
