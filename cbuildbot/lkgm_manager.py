...
class LKGMManager(manifest_version.BuildSpecsManager):
  ...
  def _AdjustRepoCheckoutToLocalManifest(self, manifest_path):
    ...
    orig_url = self.cros_source.manifest_repo_url
    try:
      self.cros_source.Initialize(manifest_repo_url=tmp_manifest_repo)

      # Note, this undoes the CLs applied into the internal manifest repository.
      # It then sync's manifest-internal to it's current TOT, which might be
      # newer than what the CL's were applied too. The manifest CLs MIGHT fail
      # to apply on build slaves and fail the CQ run quickly.

      # If this is a real issue, fix it by patching in the SHA1 from the first
      # checkout into this checkout.
      self.cros_source.Sync()
    finally:
      self.cros_source.Initialize(manifest_repo_url=orig_url)
  ...
  def CreateNewCandidate(self, validation_pool=None,
    ...
    manifest_dir = os.path.join(
        constants.SOURCE_ROOT, 'manifest-internal')

    if not os.path.exists(manifest_dir):
      # Fall back to external manifest directory.
      manifest_dir = os.path.join(
          constants.SOURCE_ROOT, 'manifest')
