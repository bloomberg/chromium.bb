...
class ManifestVersionedSyncStage(SyncStage):
  ...
  def _GetMasterVersion(self):
    """Get the platform version associated with the master_build_id."""
    master_id = self._run.options.master_build_id
    _, db = self._run.GetCIDBHandle()
    if db and master_id:
      master_build_status = db.GetBuildStatus(master_id)
      return master_build_status['platform_version']

  def _VerifyMasterId(self):
  ...
  def PerformStage(self):
    ...
    self._VerifyMasterId()
    version = self._run.options.force_version or self._GetMasterVersion()
    next_manifest = None
    if version:
...
class MasterSlaveLKGMSyncStage(ManifestVersionedSyncStage):
  ...
  def _VerifyMasterId(self):
    """Verify that our master id is current and valid."""
    super(MasterSlaveLKGMSyncStage, self)._VerifyMasterId()
    master_id = self._run.options.master_build_id
