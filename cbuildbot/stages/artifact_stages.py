...
class CollectPGOProfilesStage(generic_stages.BoardSpecificBuilderStage,
  ...
  def _CollectPGOProfiles(self):
    ...
    out_chroot_sys_devel = os.path.abspath(
        os.path.join(self._build_root, 'chroot', self.SYS_DEVEL_DIR))
    try:
      ...
      profraws = [os.path.join(profiles_dir, f)
    ...
    merge_cmd = ['llvm-profdata', 'merge', '-output', profdata_loc] + profraws
    cros_build_lib.RunCommand(merge_cmd, cwd=self._build_root,
                              enter_chroot=True)

    out_profdata_loc = os.path.join(out_chroot_path, self.PROFDATA)
    cros_build_lib.CreateTarball(self.PROFDATA_TAR, cwd=self._build_root,
