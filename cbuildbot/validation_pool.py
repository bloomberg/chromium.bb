...
class PatchSeries(object):
  ...
  @_ManifestDecorator
  def ResetCheckouts(self, ref, fetch=False):
    """Updates |ref| in all Git checkouts in the manifest to their remotes.
    ...
    if not self.manifest:
      logging.info("No manifest, skpping reset.")
      return
    ...
    def _Reset(checkout):
      ...
      git.RunGit(path, ['update-ref', ref,
                        checkout['tracking_branch']],
...
class ValidationPool(object):
  ...
  def SubmitChanges(self, changes, check_tree_open=True, throttled_ok=True,
    ...
    patch_series.ResetCheckouts(constants.PATCH_BRANCH, fetch=True)
