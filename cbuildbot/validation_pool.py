...
class CalculateSuspects(object):
  ...
  def FilterOutInnocentChanges(cls, build_root, changes, messages):
    ...
    candidates = changes
    if not any(message and message.internal for message in messages):
  ...
  def FilterOutInnocentOverlayChanges(cls, build_root, changes, messages):
    ...
    Returns:
      A list the changes that we could not prove innocent.
    """
...
class ValidationPool(object):
  ...
  def SubmitPartialPool(self, messages):
    ...
    for change in self.changes:
      ...
      if not failing_stages.issubset(ignored_stages):
