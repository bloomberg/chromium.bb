...
CHROME_EBUILD_AFDO_REGEX = (
    r'^(?P<bef>%s\["%s"\]=")(?P<name>.*)(?P<aft>")')
# and corresponding replacement string.
CHROME_EBUILD_AFDO_REPL = r'\g<bef>%s\g<aft>'

# AutoFDO profiles may come from difference sources/groups.
# Profiles in GROUP_BENCH are generated by our selection of benchmarks. This is
# used by most of the boards.
GROUP_BENCH = 'AFDO_FILE_LLVM'
# GROUP_CWP holds profiles directly from field.
...
def PatchChromeEbuildAFDOFile(ebuild_file, group, arch_profiles):
  ...
  for arch in arch_profiles.keys():
    arch_patterns[arch] = re.compile(CHROME_EBUILD_AFDO_REGEX % (group, arch))
...
def UpdateChromeEbuildAFDOFile(board, bench_profiles, cwp_profiles):
  ...
  Args:
    ...
    bench_profiles: {arch: afdo_file} pairs to put into the ebuild.
                    These are profiles from selected benchmarks.
    cwp_profiles: {arch: afdo_file} pairs to put into the ebuild.
                  These are profiles from cwp.
  """
  ...
  PatchChromeEbuildAFDOFile(ebuild_file, GROUP_BENCH, bench_profiles)
  PatchChromeEbuildAFDOFile(ebuild_file, GROUP_CWP, cwp_profiles)
  ...
  PatchChromeEbuildAFDOFile(ebuild_9999, GROUP_BENCH, bench_profiles)
  PatchChromeEbuildAFDOFile(ebuild_9999, GROUP_CWP, cwp_profiles)
...
def GetBenchmarkProfile(cpv, arch, buildroot, gs_context):
