...
TEST_F(PdfCompositorImplTest, CompleteDocumentLackingPrepare) {
  MockCompletionPdfCompositorImpl impl;

  // Frame 2 and 3 are painted.
  impl.AddSubframeContent(2, CreateTestData(2, -1), ContentToFrameMap());
  impl.AddSubframeContent(3, CreateTestData(3, -1), ContentToFrameMap());
  EXPECT_CALL(impl, OnCompositeToPdf(testing::_, testing::_)).Times(2);
  impl.CompositePageToPdf(
      3, CreateTestData(2, -1), ContentToFrameMap(),
      base::BindOnce(&PdfCompositorImplTest::OnCompositeToPdfCallback));
  impl.CompositePageToPdf(
      3, CreateTestData(3, -1), ContentToFrameMap(),
      base::BindOnce(&PdfCompositorImplTest::OnCompositeToPdfCallback));
  testing::Mock::VerifyAndClearExpectations(&impl);

  // Death from DCHECK expected, because haven't done PrepareForDocumentToPdf()
  // prior to CompositePageToPdf() and CompleteDocumentToPdf().
  EXPECT_CHECK_DEATH(impl.CompleteDocumentToPdf(
      2, base::BindOnce(
             &PdfCompositorImplTest::OnCompositeOrCompleteDocumentToPdfCallback,
             base::Unretained(this))));
}

TEST_F(PdfCompositorImplTest, CompositeDocumentAfterPrepare) {
  MockCompletionPdfCompositorImpl impl;
  ...
  // Death from DCHECK expected, because should not call
  // CompositeDocumentToPdf() after PrepareForDocumentToPdf().
  EXPECT_CHECK_DEATH(impl.CompositeDocumentToPdf(
      3, CreateTestData(2, -1), ContentToFrameMap(),
      base::BindOnce(
          &PdfCompositorImplTest::OnCompositeOrCompleteDocumentToPdfCallback,
          base::Unretained(this))));
