...
class DCTestOverlayProcessor : public OverlayProcessor {
 public:
  explicit DCTestOverlayProcessor(const ContextProvider* context_provider)
      : OverlayProcessor(context_provider) {
    SetOverlayCandidateValidator(std::make_unique<DCLayerValidator>());
  }
};
...
TEST_F(DCLayerOverlayTest, AllowNonAxisAlignedTransform) {
  ...
  overlay_processor_->ProcessForOverlays(
      ...
      render_pass_filters, render_pass_backdrop_filters, &dc_layer_list,
...
TEST_F(DCLayerOverlayTest, MultiplePassDamageRect) {
  ...
  overlay_processor_->ProcessForOverlays(
      resource_provider_.get(), &pass_list, GetIdentityColorMatrix(),
      render_pass_filters, render_pass_backdrop_filters, &dc_layer_list,
      &root_damage_rect, &content_bounds_);
  EXPECT_EQ(0U, output_surface_->bind_framebuffer_count());
