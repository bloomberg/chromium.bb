...
static int get_wide_input(widechar *buffer) {
...
static char *print_number(widechar c) {
...
static void append_char(char *destination, int *length, char source) {
  destination[(*length)++] = source;
}

static void append_string(char *destination, int *length, char *source) {
