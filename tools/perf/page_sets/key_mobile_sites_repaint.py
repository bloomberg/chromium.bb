...
class KeyMobileSitesRepaintPageSet(page_set_module.PageSet):
  ...
  def __init__(self):
    ...
    for page_class in predefined_page_classes:
      self.AddPage(_CreatePageClassWithRepaintInteractions(page_class)(self))

    # Add pages with custom labels.

    # Why: Top news site.
    self.AddPage(KeyMobileSitesRepaintPage(
   ...
   # Why: #6 (Alexa) most visited worldwide, picked an interesting page
    ...
    for url in urls_list:
      self.AddPage(KeyMobileSitesRepaintPage(url, self))
