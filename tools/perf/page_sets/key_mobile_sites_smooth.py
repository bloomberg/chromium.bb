...
class KeyMobileSitesSmoothPageSet(page_set_module.PageSet):
  ...
  def __init__(self):
    ...
    for page_class in predefined_page_classes:
      self.AddPage(_CreatePageClassWithSmoothInteractions(page_class)(self))

    # Add pages with custom page interaction logic.

    # Page behaves non-deterministically, replaced with test version for now.
    # self.AddPage(GroupClonedSmoothPage(self))
    # mean_input_event_latency cannot be tracked correctly for
    # GroupClonedListImagesPage.
    # See crbug.com/409086.
    # self.AddPage(GroupClonedListImagesSmoothPage(self))
    self.AddPage(GoogleNewsMobile2SmoothPage(self))
    self.AddPage(AmazonNicolasCageSmoothPage(self))

    # Add pages with custom labels.

    # Why: Top news site.
    self.AddPage(KeyMobileSitesSmoothPage(
   ...
   # Why: #6 (Alexa) most visited worldwide, picked an interesting page
    ...
    for url in urls_list:
      self.AddPage(KeyMobileSitesSmoothPage(url, self))
