...
# TODO(crbug.com/863460): The code in this script should be deduplicated with
# gn's build/gen.py, and this script should be changed to be a thin wrapper that
# forwards to gen.py.
...
def windows_prepare_toolchain(tempdir):
  ...
  windows_x64_toolchain = CallPythonScopeScript(
      [
          sys.executable,
          os.path.join(SRC_ROOT, "build", "toolchain", "win",
                       "setup_toolchain.py"), toolchain_paths["vs_path"],
          toolchain_paths["sdk_path"], toolchain_paths["runtime_dirs"], "win",
          "x64", "environment.x64", "true"
      ],
      cwd=tempdir)
...
def main(argv):
  ...
  parser.add_option(
      '-v', '--verbose', action='store_true', help='Log more details')
...
def build_gn_with_ninja_manually(tempdir, options, windows_x64_toolchain):
  ...
  write_buildflag_header_manually(
      root_gen_dir, 'base/synchronization/synchronization_buildflags.h',
      {'ENABLE_MUTEX_PRIORITY_INHERITANCE': 'false'})

  write_buildflag_header_manually(
      root_gen_dir, 'base/allocator/buildflags.h',
  ...
  write_buildflag_header_manually(
      root_gen_dir, 'base/cfi_buildflags.h', {
  ...
  if is_mac:
    ...
    check_call([
        sys.executable,
        os.path.join(SRC_ROOT, 'build', 'write_build_date_header.py'),
        os.path.join(root_gen_dir, 'base', 'generated_build_date.h'), 'default'
    ])

  if is_win:
    write_buildflag_header_manually(
        root_gen_dir, 'base/win/base_win_buildflags.h',
        {'SINGLE_MODULE_MODE_HANDLE_VERIFIER': 'true'})

    write_compiled_message(
        root_gen_dir, 'base/trace_event/etw_manifest/chrome_events_win.man')

  write_buildflag_header_manually(root_gen_dir, 'base/android/library_loader.h',
                                  {
                                      'USE_LLD': 'false',
                                      'SUPPORTS_CODE_ORDERING': 'false'
                                  })

  write_gn_ninja(
      os.path.join(tempdir, 'build.ninja'), root_gen_dir, options,
      windows_x64_toolchain)
...
def write_generic_ninja(path,
  ...
  def build_source(src_file, settings):
    ninja_lines.extend([
        'build %s: %s %s' % (src_to_obj(src_file), settings['tool'],
                             escape_path_ninja(
                                 os.path.join(SRC_ROOT, src_file))),
        '  includes = %s' % ' '.join([
            '-I' + escape_path_ninja(dirname)
            for dirname in include_dirs + settings.get('include_dirs', [])
        ]),
...
def write_gn_ninja(path, root_gen_dir, options, windows_x64_toolchain):
  ...
  if is_linux or is_aix:
    static_libraries['xdg_user_dirs'] = {
        'sources': ['base/third_party/xdg_user_dirs/xdg_user_dir_lookup.cc',],
        'tool': 'cxx',
    }
    static_libraries['base']['sources'].extend([
...
def build_gn_with_gn(temp_gn, build_dir, options):
  ...
  # gets updated to "./gn".
  cmd = [
      os.path.join(build_dir, 'gn'), 'gen', build_dir,
      '--args=%s' % gn_gen_args
  ]
  check_call(cmd)
