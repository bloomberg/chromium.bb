...
# LLVM r242705 changed how tools are incorporated into the LLVM build: Clang is
# now treated as an implicit tool as well. Unfortunately,
# add_llvm_implicit_projects() uses a glob expression to pick up implicit tools,
# which appears to return results in sorted order. Since the build hook for
# the Chrome tools depends on tools/clang's CMakeLists.txt being processed
# first, prefix the shim directory's name with zzz to ensure it gets processed
# last.
CHROME_TOOLS_SHIM_DIR = os.path.join(LLVM_DIR, 'tools', 'zzz-chrometools')
...
def DeleteChromeToolsShim():
  OLD_SHIM_DIR = os.path.join(LLVM_DIR, 'tools', 'chrometools')
