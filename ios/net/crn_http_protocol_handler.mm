...
HttpProtocolHandlerCore::HttpProtocolHandlerCore(NSURLSessionTask* task)
  ...
  task_.reset(task);
  request_.reset([[task currentRequest] mutableCopy]);
  // Will allocate read buffer with size |kIOBufferMinSize|.
  AllocateReadBuffer(0);
  [request_ setURL:[[task currentRequest] URL]];
}
...
+ (BOOL)canInitWithTask:(NSURLSessionTask*)task {
  DVLOG(5) << "canInitWithTask "
           << net::FormatUrlRequestForLogging(task.currentRequest);
  return g_protocol_handler_delegate->CanHandleRequest(task.currentRequest);
}
