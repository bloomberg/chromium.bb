...
class DownloadTaskImplTest : public WebTestWithWebState {
 protected:
  void SetUp() override {
    WebTestWithWebState::SetUp();
    task_ = std::make_unique<DownloadTaskImpl>(
        web_state(), GURL(kUrl), kContentDisposition,
        /*total_bytes=*/-1, kMimeType,
        task_delegate_.configuration().identifier, &task_delegate_);
    task_->AddObserver(&task_observer_);

    // -[WKHTTPCookieStore getAllCookies:] does not work if
    // WKWebViewConfiguration object was created, but WKWebView did not load a
    // URL. This problem does not exist on simulator or if the app running the
    // real NSRunLoop.
    web::test::LoadUrl(web_state(), GURL("about:blank"));
  }
