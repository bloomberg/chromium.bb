...
string SpdyUtils::GetPromisedUrlFromHeaders(const SpdyHeaderBlock& headers) {
  ...
  // Canonicalize the scheme; this is to ensure a scheme of "foo://bar" is not
...
// static
string SpdyUtils::GetPromisedHostNameFromHeaders(
...
bool SpdyUtils::PromisedUrlIsValid(const SpdyHeaderBlock& headers) {
  string url(GetPromisedUrlFromHeaders(headers));
