...
class ClientSession : public protocol::HostStub,
 ...
 public:
  ...
  // Stop all the session and deletes |this| once it is completely stopped.
  // StopAndDelete() is the only way to destoy a |ClientSession| instance.
  // |done_task| is executed when the session is completely stopped.
  void StopAndDelete(const base::Closure& done_task);
