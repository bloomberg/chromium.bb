...
def WriteFile(contents, full_path, comment_starter=None):
...
class Generator(object):
  ...
  def Write(self, contents, filename, comment_starter):
    if self.output_dir is None:
      print(contents)
      return
    full_path = os.path.join(self.output_dir, filename)
    WriteFile(contents, full_path, comment_starter)
  ...
  def WriteWithComment(self, contents, filename):
    ...
    comment = r"// %s is auto generated by %s, do not edit" % (filename,
                                                               generator_name)
    self.Write(contents, filename, comment)
