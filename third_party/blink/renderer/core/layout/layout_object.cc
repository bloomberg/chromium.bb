...
void LayoutObject::UpdateFirstLineImageObservers(
  ...
  // and we can't update the pseudo style cache.
...
const ComputedStyle* LayoutObject::CachedFirstLineStyle() const {
  ...
  if (scoped_refptr<const ComputedStyle> style =
    ...
    if (bitfields_.PendingUpdateFirstLineImageObservers()) {
      // Update image observers now after we have updated the first line style
      // cache.
