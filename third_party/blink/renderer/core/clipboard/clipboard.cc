...
Clipboard& Clipboard::GetInstance() {
  DEFINE_STATIC_LOCAL(Clipboard, clipboard, ());
  return clipboard;
}
...
bool Clipboard::IsSelectionMode() const {
  return buffer_ == mojom::ClipboardBuffer::kSelection;
}

void Clipboard::SetSelectionMode(bool selection_mode) {
  buffer_ = selection_mode ? mojom::ClipboardBuffer::kSelection
                           : mojom::ClipboardBuffer::kStandard;
}

bool Clipboard::CanSmartReplace() {
...
bool Clipboard::IsHTMLAvailable() {
  if (!IsValidBufferType(buffer_))
    return false;
  bool result = false;
...
uint64_t Clipboard::SequenceNumber() {
  if (!IsValidBufferType(buffer_))
    return 0;
...
Vector<String> Clipboard::ReadAvailableTypes() {
  Vector<String> types;
  if (IsValidBufferType(buffer_)) {
    bool unused;
...
String Clipboard::ReadPlainText() {
  return ReadPlainText(buffer_);
}

String Clipboard::ReadPlainText(mojom::ClipboardBuffer buffer) {
...
void Clipboard::WritePlainText(const String& plain_text, SmartReplaceOption) {
...
String Clipboard::ReadHTML(KURL& url,
...
void Clipboard::WriteHTML(const String& markup,
...
scoped_refptr<BlobDataHandle> Clipboard::ReadImage(
...
void Clipboard::WriteImage(Image* image, const KURL& url, const String& title) {
  DCHECK(image);
...
String Clipboard::ReadCustomData(const String& type) {
  if (!IsValidBufferType(buffer_))
    return String();
  String data;
...
void Clipboard::WriteDataObject(DataObject* data_object) {
...
bool Clipboard::IsValidBufferType(mojom::ClipboardBuffer buffer) {
  switch (buffer) {
