...
void VibrationController::doVibrate(Timer<VibrationController>* timer)
{
    ...
    if (m_service) {
        ...
        m_service->Vibrate(m_pattern[0], createBaseCallback(bind(&VibrationController::didVibrate, wrapPersistent(this))));
    }
}
...
void VibrationController::cancel()
{
    ...
    if (m_isRunning && !m_isCallingCancel && m_service) {
        ...
        m_service->Cancel(createBaseCallback(bind(&VibrationController::didCancel, wrapPersistent(this))));
    }
