...
class MockClient : public GarbageCollectedFinalized<MockClient>,
 ...
 public:
  static StrictMock<MockClient>* create() {
    return new StrictMock<MockClient>();
  }
...
class ReadableStreamDataConsumerHandleTest : public ::testing::Test {
 public:
  ReadableStreamDataConsumerHandleTest() : m_page(DummyPageHolder::create()) {}
  ...
  std::unique_ptr<ReadableStreamDataConsumerHandle> createHandle(
      ScriptValue stream) {
...
TEST_F(ReadableStreamDataConsumerHandleTest, Create) {
  ...
  std::unique_ptr<ReadableStreamDataConsumerHandle> handle =
      createHandle(stream);
  ASSERT_TRUE(handle);
  ...
  std::unique_ptr<FetchDataConsumerHandle::Reader> reader =
      handle->obtainFetchDataReader(client);
  ASSERT_TRUE(reader);
  checkpoint.Call(1);
...
TEST_F(ReadableStreamDataConsumerHandleTest, StreamReaderShouldBeWeak) {
...

  {
    ...
    ScriptState::Scope scope(getScriptState());
    stream = ScriptValue(getScriptState(),
                         evalWithPrintingError("new ReadableStream()"));
    ASSERT_FALSE(stream.isEmpty());
    std::unique_ptr<ReadableStreamDataConsumerHandle> handle =
        createHandle(stream);
...
TEST_F(ReadableStreamDataConsumerHandleTest,
       StreamReaderShouldBeWeakWhenReading) {
  std::unique_ptr<FetchDataConsumerHandle::Reader> reader;
  Checkpoint checkpoint;
  Persistent<MockClient> client = MockClient::create();
  ScriptValue stream;
