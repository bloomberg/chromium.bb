...
namespace WTF {
    ...
    {
        /*success = false;
        RELEASE_ASSERT(m_tableSize <= newTableSize);
        if (m_tableSize < newTableSize && !Allocator::expandHashTableBacking(m_table, newTableSize * sizeof(ValueType)))
            return 0;
        ...
        if (Traits::emptyValueIsZero) {
            memset(temporaryTable, 0, oldTableSize * sizeof(ValueType));
        } else {
        ...
        //newEntry = rehashTo(temporaryTable, newTableSize, entry);
