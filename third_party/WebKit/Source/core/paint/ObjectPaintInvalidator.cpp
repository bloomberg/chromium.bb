...
void traverseNonCompositingDescendantsInPaintOrder(const LayoutObject&, const LayoutObjectTraversalFunctor&);

template <typename LayoutObjectTraversalFunctor>
void traverseNonCompositingDescendantsBelongingToAncestorPaintInvalidationContainer(const LayoutObject& object, const LayoutObjectTraversalFunctor& functor)
{
...
template <typename LayoutObjectTraversalFunctor>
void traverseNonCompositingDescendantsInPaintOrder(const LayoutObject& object, const LayoutObjectTraversalFunctor& functor)
{
    functor(object);
    LayoutObject* descendant = object.nextInPreOrder(&object);
    while (descendant) {
...
void ObjectPaintInvalidator::invalidateDisplayItemClientsIncludingNonCompositingDescendants(PaintInvalidationReason reason)
{
    // This is valid because we want to invalidate the client in the display item list of the current backing.
    DisableCompositingQueryAsserts disabler;

    slowSetPaintingLayerNeedsRepaint();
    traverseNonCompositingDescendantsInPaintOrder(m_object, [reason](const LayoutObject& object) {
...
void ObjectPaintInvalidator::invalidatePaintIncludingNonCompositingDescendants()
{
    ...
    traverseNonCompositingDescendantsInPaintOrder(m_object, [&paintInvalidationContainer](const LayoutObject& object) {
        if (object.hasLayer())
            toLayoutBoxModelObject(object).layer()->setNeedsRepaint();
        ObjectPaintInvalidator(object).invalidatePaintOfPreviousPaintInvalidationRect(paintInvalidationContainer, PaintInvalidationSubtree);
    });
}

void ObjectPaintInvalidator::invalidatePaintIncludingNonSelfPaintingLayerDescendantsInternal(const LayoutBoxModelObject& paintInvalidationContainer)
{
    invalidatePaintOfPreviousPaintInvalidationRect(paintInvalidationContainer, PaintInvalidationSubtree);
    for (LayoutObject* child = m_object.slowFirstChild(); child; child = child->nextSibling()) {
        if (!child->hasLayer() || !toLayoutBoxModelObject(child)->layer()->isSelfPaintingLayer())
            ObjectPaintInvalidator(*child).invalidatePaintIncludingNonSelfPaintingLayerDescendantsInternal(paintInvalidationContainer);
    }
}

void ObjectPaintInvalidator::invalidatePaintIncludingNonSelfPaintingLayerDescendants(const LayoutBoxModelObject& paintInvalidationContainer)
{
    slowSetPaintingLayerNeedsRepaint();
    invalidatePaintIncludingNonSelfPaintingLayerDescendantsInternal(paintInvalidationContainer);
}

void ObjectPaintInvalidator::invalidateDisplayItemClient(const DisplayItemClient& client, PaintInvalidationReason reason)
{
    // It's caller's responsibility to ensure enclosingSelfPaintingLayer's needsRepaint is set.
    // Don't set the flag here because getting enclosingSelfPaintLayer has cost and the caller can use
    // various ways (e.g. PaintInvalidatinState::enclosingSelfPaintingLayer()) to reduce the cost.
...
void ObjectPaintInvalidator::setBackingNeedsPaintInvalidationInRect(const LayoutBoxModelObject& paintInvalidationContainer, const LayoutRect& rect, PaintInvalidationReason reason)
{
    ...
    if (layer.groupedMapping()) {
        if (GraphicsLayer* squashingLayer = layer.groupedMapping()->squashingLayer()) {
            ...
            squashingLayer->setNeedsDisplayInRect(enclosingIntRect(rect), reason, m_object);
        }
        ...
        layer.compositedLayerMapping()->setNonScrollingContentsNeedDisplayInRect(rect, reason, m_object);
    } else {
        // Otherwise invalidate everything.
        layer.compositedLayerMapping()->setContentsNeedDisplayInRect(rect, reason, m_object);
    }
}

void ObjectPaintInvalidator::invalidatePaintUsingContainer(const LayoutBoxModelObject& paintInvalidationContainer, const LayoutRect& dirtyRect, PaintInvalidationReason invalidationReason)
{
    ...
    DCHECK(gDisablePaintInvalidationStateAsserts
        || m_object.document().lifecycle().state() == (RuntimeEnabledFeatures::slimmingPaintInvalidationEnabled() ? DocumentLifecycle::InPrePaint : DocumentLifecycle::InPaintInvalidation));
    ...
    TRACE_EVENT_INSTANT1(TRACE_DISABLED_BY_DEFAULT("devtools.timeline.invalidationTracking"),
        ...
        "data", InspectorPaintInvalidationTrackingEvent::data(&m_object, paintInvalidationContainer));
    TRACE_EVENT2(TRACE_DISABLED_BY_DEFAULT("blink.invalidation"), "LayoutObject::invalidatePaintUsingContainer()",
        "object", m_object.debugName().ascii(),
...
bool ObjectPaintInvalidatorWithContext::incrementallyInvalidatePaint()
{
    ...
    if (deltaRight > 0) {
        ...
        invalidatePaintUsingContainer(*m_context.paintInvalidationContainer, invalidationRect, PaintInvalidationIncremental);
...
void ObjectPaintInvalidatorWithContext::fullyInvalidatePaint(PaintInvalidationReason reason, const LayoutRect& oldBounds, const LayoutRect& newBounds)
{
    ...
    if (!newBounds.contains(oldBounds)) {
        ...
        invalidatePaintUsingContainer(*m_context.paintInvalidationContainer, invalidationRect, reason);

        if (invalidationRect.contains(newBounds))
            return;
    }

    invalidatePaintUsingContainer(*m_context.paintInvalidationContainer, newBounds, reason);
}

PaintInvalidationReason ObjectPaintInvalidatorWithContext::computePaintInvalidationReason()
{
...
void ObjectPaintInvalidatorWithContext::invalidateSelectionIfNeeded(PaintInvalidationReason reason)
{
...
PaintInvalidationReason ObjectPaintInvalidatorWithContext::invalidatePaintIfNeededWithComputedReason(PaintInvalidationReason reason)
{
