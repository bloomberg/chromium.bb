...
void BlockFlowPaintInvalidator::invalidatePaintForOverhangingFloatsInternal(InvalidateDescendantMode invalidateDescendants)
{
    ...
    for (const auto& floatingObject : m_blockFlow.floatingObjects()->set()) {
        ...
        if (m_blockFlow.isOverhangingFloat(*floatingObject)
            ...
            && (floatingObject->shouldPaint() || (invalidateDescendants == InvalidateDescendants && floatingObject->layoutObject()->isDescendantOf(&m_blockFlow)))) {
            ...
            if (floatingBox->isLayoutBlockFlow())
                BlockFlowPaintInvalidator(*toLayoutBlockFlow(floatingBox)).invalidatePaintForOverhangingFloatsInternal(DontInvalidateDescendants);
        }
    }
}

void BlockFlowPaintInvalidator::invalidateDisplayItemClients(PaintInvalidationReason reason)
{
    ...
    if (m_blockFlow.isAnonymous()) {
        ...
        if (startOfContinuations && startOfContinuations->styleRef().outlineStyleIsAuto())
            ObjectPaintInvalidator(*startOfContinuations).slowSetPaintingLayerNeedsRepaintAndInvalidateDisplayItemClient(*startOfContinuations, reason);
    }
