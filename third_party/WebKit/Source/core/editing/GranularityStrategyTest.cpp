...
void GranularityStrategyTest::setupTextSpan(String str1, String str2, String str3, size_t selBegin, size_t selEnd)
{
    ...
    selection().setSelection(createVisibleSelection(p1, p2));
}
...
TEST_F(GranularityStrategyTest, DirectionExpandTranslateZ)
{
    ...
    selection().setSelection(createVisibleSelection(Position(text, 18), Position(text, 19)));
    EXPECT_EQ_SELECTED_TEXT("p");
    testDirectionExpand();
}
...
TEST_F(GranularityStrategyTest, DirectionShrinkTranslateZ)
{
    ...
    selection().setSelection(createVisibleSelection(Position(text, 18), Position(text, 21)));
    EXPECT_EQ_SELECTED_TEXT("pqr");
    testDirectionShrink();
}
...
TEST_F(GranularityStrategyTest, DirectionSwitchStartOnBoundary)
{
    ...
    // > means end).
    selection().setSelection(createVisibleSelection(Position(text, 15), Position(text, 22)));
