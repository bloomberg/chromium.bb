...
CompositionUnderline createCompositionUnderline(unsigned startOffset, unsigned endOffset)
{
    return CompositionUnderline(startOffset, endOffset, Color::transparent, false, Color::transparent);
}
...
TEST(CompositionUnderlineTest, LastChar)
{
    CompositionUnderline underline = createCompositionUnderline(
        std::numeric_limits<unsigned>::max() - 1, std::numeric_limits<unsigned>::max());
...
{
    CompositionUnderline underline = createCompositionUnderline(
        std::numeric_limits<unsigned>::max(), std::numeric_limits<unsigned>::max() - 1);
    EXPECT_EQ(std::numeric_limits<unsigned>::max() - 1, underline.startOffset());
    EXPECT_EQ(std::numeric_limits<unsigned>::max(), underline.endOffset());
}
