...
static String toCJKIdeographic(int number,
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    if (digitValue) {
      bool dropOne = table[Lang] == Korean && cjkStyle == Informal &&
                     digitValue == 1 && i > 0;
      if (!dropOne)
    ...
    if (number != 0 || groupValue > 9) {
      ...
      bool dropOne =
          table[Lang] == Korean && cjkStyle == Informal && digitValue == 1;
    ...
    if (a != NoChar) {
      if (a != Digit0 || (table[Lang] == Chinese && last != Digit0)) {
        ...
        if (newChar != NoChar) {
          ...
          if (table[Lang] == Korean &&
              (a == SecondGroupMarker || a == ThirdGroupMarker ||
               a == FourthGroupMarker))
            characters[length++] = ' ';
        }
      }
  ...
  if ((table[Lang] == Chinese && last == Digit0) ||
      characters[length - 1] == ' ')
    --length;
