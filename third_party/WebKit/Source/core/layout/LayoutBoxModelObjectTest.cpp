...
protected:
    const FloatRect& getScrollContainerRelativeContainingBlockRect(const StickyPositionScrollingConstraints& constraints) const
    {
        return constraints.scrollContainerRelativeContainingBlockRect();
    }

    const FloatRect& getScrollContainerRelativeStickyBoxRect(const StickyPositionScrollingConstraints& constraints) const
    {
...
{
    setBodyInnerHTML("<style>#sticky { position: sticky; margin-top: 10%; top: 0; width: 100px; height: 100px; }"
        "#container { box-sizing: border-box; position: relative; top: 100px; height: 400px; width: 250px; padding: 5%; border: 5px solid black; }"
        "#scroller { width: 400px; height: 100px; overflow: auto; position: relative; top: 200px; }"
        ".spacer { height: 1000px; }</style>"
        "<div id='scroller'><div id='container'><div id='sticky'></div></div><div class='spacer'></div></div>");
    LayoutBoxModelObject* scroller = toLayoutBoxModelObject(getLayoutObjectByElementId("scroller"));
    scroller->getScrollableArea()->scrollToYOffset(50);
    ...
    ASSERT_EQ(IntRect(25, 145, 200, 330), enclosingIntRect(getScrollContainerRelativeContainingBlockRect(constraints)));
    ASSERT_EQ(IntRect(25, 145, 100, 100), enclosingIntRect(getScrollContainerRelativeStickyBoxRect(constraints)));
}
...
TEST_F(LayoutBoxModelObjectTest, StickyPositionContainerIsScroller)
{
    ...
    const StickyPositionScrollingConstraints& constraints = scroller->getScrollableArea()->stickyConstraintsMap().get(sticky->layer());
    ASSERT_EQ(IntRect(0, 0, 400, 1100), enclosingIntRect(getScrollContainerRelativeContainingBlockRect(constraints)));
    ASSERT_EQ(IntRect(0, 0, 100, 100), enclosingIntRect(getScrollContainerRelativeStickyBoxRect(constraints)));
}
...
TEST_F(LayoutBoxModelObjectTest, StickyPositionAnonymousContainer)
{
    ...
    const StickyPositionScrollingConstraints& constraints = scroller->getScrollableArea()->stickyConstraintsMap().get(sticky->layer());
    ASSERT_EQ(IntRect(15, 115, 170, 370), enclosingIntRect(getScrollContainerRelativeContainingBlockRect(constraints)));
    ASSERT_EQ(IntRect(15, 165, 100, 100), enclosingIntRect(getScrollContainerRelativeStickyBoxRect(constraints)));
}
} // namespace blink
