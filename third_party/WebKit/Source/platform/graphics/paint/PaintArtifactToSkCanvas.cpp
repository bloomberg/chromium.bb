...
void paintDisplayItemToSkCanvas(const DisplayItem& displayItem,
                                SkCanvas* canvas) {
...
static void computePathsToCommonAncestor(
    const EffectPaintPropertyNode* a,
    const EffectPaintPropertyNode* b,
    Vector<const EffectPaintPropertyNode*>& aToCommonAncestor,
    Vector<const EffectPaintPropertyNode*>& bToCommonAncestor) {
  ...
  // ancestor itself.
  while (aToCommonAncestor.size() && bToCommonAncestor.size() &&
         aToCommonAncestor.last() == bToCommonAncestor.last()) {
    aToCommonAncestor.removeLast();
    bToCommonAncestor.removeLast();
  }
}

static void applyEffectNodesToCanvas(
    const EffectPaintPropertyNode* previousEffect,
  ...
  computePathsToCommonAncestor(previousEffect, currentEffect, effectsToUnapply,
                               effectsToApply);
  ...
  for (Vector<const EffectPaintPropertyNode*>::reverse_iterator it =
           effectsToApply.rbegin();
       it != effectsToApply.rend(); ++it) {
...
static TransformationMatrix totalTransform(
    const TransformPaintPropertyNode* currentSpace) {
...
void paintArtifactToSkCanvas(const PaintArtifact& artifact, SkCanvas* canvas) {
  ...
  for (const PaintChunk& chunk : artifact.paintChunks()) {
    ...
    for (const ClipPaintPropertyNode* currentClipNode =
             chunk.properties.clip.get();
         currentClipNode; currentClipNode = currentClipNode->parent()) {
      canvas->setMatrix(TransformationMatrix::toSkMatrix44(
          totalTransform(currentClipNode->localTransformSpace())));
    ...
    TransformationMatrix combinedMatrix =
        totalTransform(chunk.properties.transform.get());
...
sk_sp<const SkPicture> paintArtifactToSkPicture(const PaintArtifact& artifact,
                                                const SkRect& bounds) {
