...
class UprevChromeCommand(command.CliCommand):
  ...
  def ValidatePFQBuild(self, pfq_build, db):
    ...
    Returns:
      True if this pfq build_id is valid for an uprev;
      Else, False.
    """
  ...
  def CommitMessage(self, body, cq_depend=None, change_id=None):
  ...
  def UprevChrome(self, work_dir, pfq_build):
    ...
    commit_message = self.CommitMessage(pub_commit_body)
    ...
    # Add CQ-DEPEND
    commit_message = self.CommitMessage(priv_commit_body, pub_cid)
    ...
    commit_message = self.CommitMessage(
        pub_commit_body, '*' + priv_cid, pub_cid)
