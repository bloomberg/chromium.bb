...
class ChromeSDKCommand(command.CliCommand):
  ...
  def AddParser(cls, parser):
    ...
    parser.add_argument(
        '--nogn-gen', action='store_false', default=True, dest='gn_gen',
        help='Skips running "gn gen" (warns if args.gn is stale).')
    parser.add_argument(
  ...
  def _UpdateGnArgsIfStale(self, gn_args, gn_args_file_path, board):
  ...
  def _SetupEnvironment(self, board, sdk_ctx, options, goma_dir=None,
    ...
    checkout_dir = (options.chrome_src if options.chrome_src else os.getcwd())
