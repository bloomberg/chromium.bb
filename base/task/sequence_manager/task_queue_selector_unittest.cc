...
class TaskQueueSelectorTest : public testing::Test {
 public:
  ...
  std::vector<size_t> PopTasksAndReturnEnqueueOrders() {
    std::vector<size_t> order;
    while (true) {
      WorkQueue* chosen_work_queue = selector_.SelectWorkQueueToService();
      if (!chosen_work_queue)
      ...
      EXPECT_TRUE(chosen_work_queue->GetFrontTaskEnqueueOrder(&enqueue_order));
      order.push_back(static_cast<size_t>(enqueue_order));
      chosen_work_queue->PopTaskForTesting();
      chosen_work_queue->work_queue_sets()->OnPopQueue(chosen_work_queue);
    }
    return order;
...
TEST_F(TaskQueueSelectorTest, TestDisableEnable) {
  ...
  EXPECT_THAT(PopTasksAndReturnQueueIndices(), testing::ElementsAre(0, 1, 3));
  ...
  selector_.SetQueuePriority(task_queues_[2].get(),
                             TaskQueue::kBestEffortPriority);
  EXPECT_THAT(PopTasksAndReturnQueueIndices(), testing::ElementsAre(2));
  task_queues_[4]->SetQueueEnabledForTest(true);
  selector_.EnableQueue(task_queues_[4].get());
  EXPECT_THAT(PopTasksAndReturnQueueIndices(), testing::ElementsAre(4));
}
...
TEST_F(TaskQueueSelectorTest, DelayedAndImmediateRoundRobin_OlderDelayedTasks) {
  for (int i = 0; i < 5; i++) {
    task_queues_[0]->delayed_work_queue()->Push(
        Task(PostedTask(test_closure_, FROM_HERE), TimeTicks(), EnqueueOrder(),
             EnqueueOrder::FromIntForTesting(1 + i)));
  }
  for (int i = 0; i < 5; i++) {
    task_queues_[0]->immediate_work_queue()->Push(
        Task(PostedTask(test_closure_, FROM_HERE), TimeTicks(), EnqueueOrder(),
             EnqueueOrder::FromIntForTesting(11 + i)));
  }

  EXPECT_THAT(PopTasksAndReturnEnqueueOrders(),
              testing::ElementsAre(1, 11, 2, 12, 3, 13, 4, 14, 5, 15));
}

TEST_F(TaskQueueSelectorTest,
       DelayedAndImmediateRoundRobin_OlderImmediateTasks) {
  for (int i = 0; i < 5; i++) {
    task_queues_[0]->immediate_work_queue()->Push(
        Task(PostedTask(test_closure_, FROM_HERE), TimeTicks(), EnqueueOrder(),
             EnqueueOrder::FromIntForTesting(1 + i)));
  ...
  // Delayed tasks are not allowed to skip ahead of immediate tasks.
  EXPECT_THAT(PopTasksAndReturnEnqueueOrders(),
              testing::ElementsAre(1, 2, 3, 4, 5, 11, 12, 13, 14, 15));
}
