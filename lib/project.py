...
class Project(object):
  ...
  def __init__(self, project_dir, initial_config=None):
    """Instantiates a project object.

    Args:
      project_dir: the root directory of the project
      initial_config: the initial configuration as a python dictionary.
        If not None, creates the project with the configuration.
    ...
    if not os.path.exists(project_json):
      ...
      raise ProjectAlreadyExists('Project creation failed. Project %s '
                                 'already exists.' % self.project_dir)
    else:
  ...
  def _WriteLayoutConf(self, content):
    ...
    osutils.WriteFile(os.path.join(self.project_dir, 'metadata', 'layout.conf'),
                      content_str, makedirs=True)
