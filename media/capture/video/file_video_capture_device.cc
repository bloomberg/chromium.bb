...
bool Y4mFileParser::Initialize(media::VideoCaptureFormat* capture_format) {
  ...
  file_->Read(0, &header[0], base::checked_cast<int>(header.size()));
  ...
  frame_size_ = base::checked_cast<int>(capture_format->ImageAllocationSize());
  return true;
}
...
bool MjpegFileParser::Initialize(media::VideoCaptureFormat* capture_format) {
  ...
  frame_size_ = base::checked_cast<int>(result.image_size);
