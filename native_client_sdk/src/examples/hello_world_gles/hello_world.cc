...
// Post Message
//
// Provide a "printf" style function which sends a message to JavaScript.
//-----------------------------------------------------------------------------
void PostMessage(const char *fmt, ...) {
  va_list args;
  va_start(args, fmt);
...
// MainLoop
//
// The main loop either waits for the three assests (two shaders and texture)
// to load by requesting a callback via CallOnMain thread.,  Once the assets
// are loaded, it finishes initialize the scene and proceeds to render
// asking for a callback when the video buffers are swapped.
//-----------------------------------------------------------------------------
void MainLoop(void* cookie, int result) {
  if (g_LoadCnt == 3) {
    InitProgram();
    g_LoadCnt++;
  }
...
// InitGL
//
// Basic OpenGL ES initialization, asking for a 640x480 rendering surface
// with 24 bit color and 8 bit alhpa.
//-----------------------------------------------------------------------------
void InitGL(void) {
  int32_t attribs[] = {
...
// CompileShader
//
// Takes the loaded vertex or fragment shader code, and compiles it.
//-----------------------------------------------------------------------------
...
void Render(void) {
  ...
  glBindBuffer(GL_ARRAY_BUFFER, g_vboID);
  glVertexAttribPointer(g_positionLoc, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex),
                        (void*)offsetof(Vertex,loc));
  glEnableVertexAttribArray(g_positionLoc);
  glVertexAttribPointer(g_texCoordLoc, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex),
                        (void*)offsetof(Vertex,tu));
  glEnableVertexAttribArray(g_texCoordLoc);
  glVertexAttribPointer(g_colorLoc, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex),
...
// 
// At this point the module is "ready" so we begin requesting resources int the
...
//
// We wait for the first DidChangeView mesage to start initializing GL to
...
//-----------------------------------------------------------------------------
// Entry point for the module, provides the module ID and a function pointer
// for requesting Browser interfaces.
//
// We take this opportunity to try and get the various interfaces we need and
// immediately fail if we can not.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Returns the plugin side interface pointer for the interface of the provided
// name, or NULL if the interface is not supported.
//-----------------------------------------------------------------------------
